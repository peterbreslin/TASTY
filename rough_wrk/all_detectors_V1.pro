; Plotting and cleaning up data files -------> VERSION 1 (with holes, zeros, and over one day only)
...
; Note: Need to remove zero values from data ans sort by wavelength
...
tsis = uniq(data.nominal_date_jdn)  ;contains all wavelengths
...
match, data[tsis].nominal_date_jdn, uv_jd[sorce_uv_0], tsispos, sorcepos_uv, epsilon=0.01
...
match, data[tsis].nominal_date_jdn, ir_jd[sorce_ir_0], tsispos, sorcepos_ir, epsilon=0.01
...
ir_refpos_sorce =  where(sorce_ir eq ref_day and ir.minwavelength ge 952.0 and ir.maxwavelength le 1598.0, count)
...
match, data[tsis].nominal_date_jdn, vis_jd[sorce_vis_0], tsispos, sorcepos_vis, epsilon=0.01
...
match, data[tsis].nominal_date_jdn, esr_jd[sorce_esr_0], tsispos, sorcepos_esr, epsilon=0.01
...
G  =  plot(esr[esr_refpos_sorce].minwavelength, esr[esr_refpos_sorce].irradiance, color='b',$
  linestyle='-.', /over)
